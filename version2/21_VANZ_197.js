...
webform.validators.vanz21 = function (v, allowOverpass) {
    ...
    //Start 26-002
                ...
                //       var col3 = toFloat(values["CAP1_R10_C3"]).toPrecision(4);
                //       col3 = Number(col3); 
    ...
    //         //values["CAP1_R10_C3"] = 0.00;/*

    //         // Wrong field format: float number needed
    //       //  document.getElementById("CAP1_R10_C3").value = "";
    // }
...
//            else 
        ...
        if (col1 > 0 && col2 > 0) {
            var col2DevCol1 = Math.round((col2 / col1) * 100) / 100;
            //Trebuie de convertit  col2DevCol1 in numa natural si dupa de atribuit
             
            
            // values["CAP1_R10_C3"] = col2DevCol1;

           // values["CAP1_R10_C3"] = parseInt(col2DevCol1);
            
          // values["CAP1_R10_C3"] = parseFloat(col2DevCol1).toFixed(2);

         // It is correct ? 
            document.getElementById("CAP1_R10_C3").value = parseFloat(col2DevCol1).toFixed(2);


            document.querySelector("#CAP1 tr:nth-child(1) td:nth-child(5)").innerHTML = col2DevCol1;

        //    // var col3 = Number(col2DevCol1.toFixed(2));
    ...
    for (var i = 10; i <= 136; i++) {
        {
            if (fun_row_26_024(i)) {
            if (!isNaN(Number(values["CAP1_R" + i + "_C4"])) && !isNaN(Number(values["CAP1_R" + i + "_C5"]))
                && !isNaN(Number(values["CAP1_R" + i + "_C6"]))
                && !isNaN((Number(values["CAP1_R" + i + "_C5"]) / Number(values["CAP1_R" + i + "_C4"])))
            ) {
                ...
                var col6 = toFloat(values["CAP1_R" + i + "_C6"]).toPrecision(3);
                ...
                if (col4 > 0 && col5 > 0) {
                    ...
                    var col5DevCol4 = Math.round((col5 / notZero(col4)) * 100) / 100;
                }
                ...
                if (col5DevCol4 != col6 && col4 != 0 && col5 != 0) {
                    webform.errors.push({
                        ...
                        'weight': 10,
                        'msg': Drupal.t('Cod eroare: 26-003 Cap.I COL6 = COL5/COL4  -  @col6 <> @col5DevCol4  ', { "@col6": col6, "@col5DevCol4": col5DevCol4 })
                    });
                }
            }
        }
